...
def main():
    print "calculating"
    knobs = ["res","fr","scr",'conn']
    budget_max = 1866.0
    budget_min = 1261.0
    config_list = parseCSV(knobs) #a list of items{config:dict,cost:float}
    optimal_counts,optimals,loss_if_not_custom = count(budget_max, budget_min, config_list,knobs)
...
def beautify_to_csv(optimal_counts,loss_if_not_custom):
    ...
    for i in range(len(optimal_counts)):
        output.write(str(i))
        output.write(',')
        output.write(str(optimal_counts[i])+","+str(loss_if_not_custom[i]['mean'])+","+str(loss_if_not_custom[i]['max'])+","+str(loss_if_not_custom[i]['std']))
        output.write('\n')
    output.close()

def count(budget_max, budget_min, config_list, knobs):
    budget_list = numpy.arange(budget_min+0.1, budget_max+1, (budget_max-budget_min) / (20.0-1.0))
    ...
    for budget in budget_list:
        ...
        priorities_list = list(itertools.product(priority_list,repeat=len(knobs)))
        cur_loss_if_not_custom = []
        # get the optimal config using default priority
        default_config, default_qos = optimal(budget,config_list,default_priorities_list)
        ...
        for cur_priorities in priorities_list:
            ...
            optimal_config,custom_qos = optimal(budget,config_list,priorities)
            ...
            cur_loss_if_not_custom.append((custom_qos-default_custom_qos) / default_custom_qos)
        ...
        #print optimals
        loss_if_not_custom.append({'max':numpy.max(cur_loss_if_not_custom),'mean':numpy.mean(cur_loss_if_not_custom),'std':numpy.std(cur_loss_if_not_custom)})
...
def qos(config_map, priorities):
    ...
    for name,value in config_map.items():
        if name=="conn":
            ...
            qos+=float(value)/720.0*priorities[name]/normalized_total
        elif name=="fr":
            qos+=float(value)/4.0*priorities[name]/normalized_total
        elif name=="scr":
            qos+=float(value)/4.0*priorities[name]/normalized_total
    return qos
...
def parseCSV(knobs):
    with open("/home/liuliu/Research/rapidlib-linux/modelConstr/data/videoPort/data.csv") as nav_csv:
            ...
            for item in row:
                if item in knobs:#this is a knob
                    ...
                    try:
                        ...
                        cost = float(item)
            config_list.append({'config_name':configuration,'config_map':config_map,'cost':cost})
        print config_list
        return config_list
