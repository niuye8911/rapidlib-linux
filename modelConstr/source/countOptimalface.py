...
def qos(config_map, priorities):
    ...
    for name,value in config_map.items():
        if name=="eyes":
            qos+=(float(value)-1)*0.5*priorities[name]/normalized_total
        elif name=="pyramid":
...
def parseCSV(knobs):
    with open("/home/liuliu/Research/rapidlib-linux/modelConstr/data/facePort/facts/fact_tx_cont.csv") as nav_csv:
