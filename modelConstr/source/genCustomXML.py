...
table = "lsh"
query_path = "/home/liuliu/Research/mara_bench/parsec-3.0/pkgs/apps/ferret/run/queriesnative"

obj_path = '/home/liuliu/Research/mara_bench/parsec-3.0/pkgs/apps/ferret/inst/amd64-linux.gcc-serial/bin/ferret'
...
def getQoS(missionfile):
        ...
        for query_image in truthmap:
            ...
            # two Sub QoS
            coverage = -2 * (len(truth_res)-len(Z)) * (len(truth_res)+1) / float(maxError)
            ranking = -1 *(compute2(truth_res, mission_res, Z) - compute1(truth_res, S) - compute1(
...
def run(preferences):
    ...
    os.system(" ".join(cmd))
    # run ferret
    output_name = "custom_outputs/output"+"_"+str(preferences[0])+"_"+str(preferences[1])+".txt"
    ferret_cmd = [obj_path,
            database_path,
            table, query_path,
            "50",
