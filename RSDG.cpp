...
vector< vector<edge_t>> *Node::getSchemeEdges()
{
    return &edges;
}
// Gets a list of dependencies from xml representation
vector<vector<xml_edge_t>> *Node::getXMLEdges(){
    return &xml_edges;
}
...
void Basic::setValue(double val)
{
    value = val;
}
...
void Level::addBasicNode(Basic *basic)
{
    basic_nodes.push_back(basic);
}
// Get a pointer to the list of basic node ptrs
vector<Basic *> *Level::getBasicNodes()
{
    return &basic_nodes;
}
...
vector<Level *> *Top::getLevelNodes()
{
    return &level_nodes;
}

// Add a level node to list of levels
void Top::addLevelNode(Level *level)
{
...
Node * RSDG::getNodeFromIndex(index_t *idx)
{
    ...
    { //looking for node that's not there
        return NULL;
    }
    return NULL;
}
