...
void regService(void* mission, char* service_name, char* node_name, void*(*func)(void*), int single, void* para, int para_value){
	...
	rsdgPara* para_cpp = (rsdgPara*) para;	
	targetMission->regService(service, node, func, single_or_not, make_pair(para_cpp, para_value));
}
...
int isFailed(void* mission){
	rsdgMission* target_mission = (rsdgMission*) mission;
	if( target_mission->isFailed() == 1) return 1;
	return 0;
}

void setUpdate(void* mission, int up){
	rsdgMission* target_mission = (rsdgMission*) mission;
	if(up==1)target_mission -> setUpdate(true);
	else target_mission -> setUpdate(false);
}
